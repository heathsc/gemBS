...
void setup_fseg_pen(int ng,int n_all)
{
	...
	if(!(order=malloc(sizeof(int)*ng*9))) ABT_FUNC(MMsg);
	olist=order+ng;
	gflag=olist+ng;
	gp_inv=gflag+ng;
	tt=gp_inv+ng;
	...
	if(!(tmp_xx=malloc(sizeof(double)*n_all))) ABT_FUNC(MMsg);
}
...
double fseg_pen(int *state[2],double **obspen,int *grps,double **freq,int nobs,int ng,int n_all)
{
	int i,j,k,k1,k2,o1,o2,o3,n_gen,no,tri_piv,dual_piv;
	int i1,i2,j1,n_inv,n_peel,n_rf,n_out,nc=0,*inv,*inv1;
	...
	for(i=0;i<nobs;i++) {
		...
		o2=state[1][i];
		if(!gpen[o1]) {
			if(!(gpen[o1]=malloc(sizeof(double)*n_all))) ABT_FUNC(MMsg);
			pd1=freq[grps[o1]];
			pd2=gpen[o1];
			for(j=0;j<n_all;j++) pd2[j]=pd1[j];
		}
		if(!gpen[o2]) {
			if(!(gpen[o2]=malloc(sizeof(double)*n_all))) ABT_FUNC(MMsg);
			pd1=freq[grps[o2]];
			pd2=gpen[o2];
			for(j=0;j<n_all;j++) pd2[j]=pd1[j];
		}
	...
	for(no=i=0;i<ng;i++) if(gpen[i]) {
		if(!order[i]) {
#ifdef TRACE
		if(CHK_TRACE(TRACE_LEVEL_2)) printf("Peeling %d->*\n",i);
#endif
			pd1=gpen[i];
			for(z=j=0;j<n_all;j++) z+=pd1[j];
			l+=log(z);
			free(gpen[i]);
		} else olist[no++]=i;
	}
...
/*		break; */
		...
		if(i<no) {
			...
			if(order[o2]>1) {
				...
				if(o2<o1) {
					for(k1=j=0;j<n_all;j++) {
						z=0.0;
						for(k=0;k<n_all;k++) z+=pd1[k]*pd12[k1++];
						zz+=pd2[j]*=z;
					}
				} else {
					for(j=0;j<n_all;j++) {
						z=0.0;
						k1=j;
						for(k=0;k<n_all;k++,k1+=n_all) z+=pd1[k]*pd12[k1];
						zz+=pd2[j]*=z;
					}
				}
				if(zz<RESCALE_LIMIT) {
				...
				if(o1<o2) {
					for(k1=j=0;j<n_all;j++) {
						z=0.0;
						for(k=0;k<n_all;k++) z+=pd2[k]*pd12[k1++];
						z1+=pd1[j]*z;
					}
				} else {
					for(j=0;j<n_all;j++) {
			...
			if(order[o2]==2 && order[o3]==2) {
				...
				pd3=gpen[o3];
				z=0.0;
				for(i=i1=0;i<n_all;i++) {
					z1=pd1[i];
					for(j1=j=0;j<n_all;j++,i1++) {
						z2=pd2[j]*pd12[i1];
						i2=i*n_all;
						for(k=0;k<n_all;k++,j1++,i2++) {
				...
				zz=0.0;
				if(o3<o1) {
					for(k=k1=0;k<n_all;k++) {
						z2=0.0;
						for(i=i1=0;i<n_all;i++,k1++) {
							z=pd1[i]*pd13[k1];
							j1=k*n_all;
							for(j=0;j<n_all;j++,i1++,j1++) z2+=z*pd2[j]*pd12[i1]*pd23[j1];
						}
						zz+=pd3[k]*=z2;
					}
				} else if(o3<o2) {
					...
					for(k=0;k<n_all;k++) {
						...
						for(i=i1=0;i<n_all;i++,k1+=n_all) {
							z=pd1[i]*pd13[k1];
							j1=k;
							for(j=0;j<n_all;j++,i1++,j1+=n_all) z2+=z*pd2[j]*pd12[i1]*pd23[j1];
						}
						zz+=pd3[k]*=z2;
					}
				}
				...
				zz=0.0;
				if(o1<o2) {
					for(j=j1=0;j<n_all;j++) {
						for(k=0;k<n_all;k++,j1++) {
							z2=0.0;
							i1=j;
							k1=k;
							for(i=0;i<n_all;i++,i1+=n_all,k1+=n_all) z2+=pd1[i]*pd12[i1]*pd13[k1];
							zz+=pd23[j1]*=z2;
						}
					}
				} else if(o1<o3) {
					for(j=j1=0;j<n_all;j++) {
						for(k=0;k<n_all;k++,j1++) {
							...
							i1=j*n_all;
							k1=k;
							for(i=0;i<n_all;i++,i1++,k1+=n_all) z2+=pd1[i]*pd12[i1]*pd13[k1];
							zz+=pd23[j1]*=z2;
						}
					}
				} else {
					for(j=j1=0;j<n_all;j++) {
						for(k=k1=0;k<n_all;k++,j1++) {
							z2=0.0;
							i1=j*n_all;
							for(i=0;i<n_all;i++,i1++,k1++) z2+=pd1[i]*pd12[i1]*pd13[k1];
							zz+=pd23[j1]*=z2;
						}
					}
				}
	...
	if(no) {
		...
		for(;;) {
			...
			while(rf1) {
				inv=rf1->inv;
				for(k2=1,k1=rf1->n_inv-1;k1>=0;k1--,k2*=n_all) {
					i2=inv[k1];
					fact[i2][k]=k2;
					rfx[i2][rfxn[i2]++]=k;
				}
			...
			for(k=1,i=n_peel;i<n_inv;i++,k*=n_all) ofact[i]=k;
			...
			if(n_out) {
				...
				if(n_out==1) {
					pd1=gpen[gp_inv[n_inv-1]];
					z1=0.0;
					for(i=0;i<n_all;i++) z1+=(pd1[i]*=xx[i]);
					xx=pd1;
				}
				if(z1<RESCALE_LIMIT) {
					l+=log(z1);
